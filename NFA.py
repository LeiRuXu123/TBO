    ...
    def transition(self, event):
        ...
        if self.state == 'AWAL':
            ...
            if event == 'nilai_memenuhi':
                self.state = 'DITERIMA'
            elif event == 'pengecekan_tambahan_diperlukan':
                self.state = 'PENGECEKAN_TAMBAHAN'
            else:
                self.state = 'DITOLAK'
        
        elif self.state == 'PENGECEKAN_TAMBAHAN':
...
def screening_mahasiswa(mahasiswa):
    ...
    if count_DITERIMA_grades >= 3:
        ...
        if algoritma_nilai in DITERIMA_grades:
            fsa.transition('nilai_tambahan_memenuhi')
        else:
            fsa.transition('nilai_tambahan_tidak_memenuhi')
    else:
        fsa.transition('nilai_tidak_memenuhi')
